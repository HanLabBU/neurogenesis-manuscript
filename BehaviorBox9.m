classdef BehaviorBox9 < hgsetget
    % ---------------------------------------------------------------------
    % BehaviorBox
    % Han Lab
    % 7/11/2011
...
%         JZ 12/17/2011
    ...
    methods % Initialization
        function obj = BehaviorBox9(varargin)
            ...
            obj.default = struct(...
                ...
                'punishTime',10,...%
                'laserTime',10,...% add laserTime so long that it will stop only after the touch 12/17/11
                'stimSet',{{[2 6];[6 2]}},...% design for trainging widnows in [2 6]
                'correctResponse',{{2;6}},...
    ...
    methods % User Control Functions
        function start(obj)    
            ...
            obj.currentStimNumber = input('Please enter the rewarded stimNumber (1 for window2;2 for window6):');% input the starting rewarded window JZ 10/11/11
            obj.rewardPump('on') % works in reverse. so ON == OFF
        ...
        function falsePokeFcn(obj,src,evnt)
            if obj.isready
                if strcmp('stimulus',obj.trialPhase)
                    obj.mouseResponse = eval(src.name(end)); % e.g. name = 'rectangle5' -> mouseResponse = 5
                    fprintf('\b %d \n',obj.mouseResponse)
                    stop(obj.stimulusTimer); %  set the false poke to be punished after stimOn JZ 12/20
