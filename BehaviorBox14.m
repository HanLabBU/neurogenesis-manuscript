classdef BehaviorBox14 < hgsetget
    % ---------------------------------------------------------------------
    % BehaviorBox
    % Han Lab
    % 7/11/2011
...
%         
%         JZ 12/17/2011
%  stage 14 for light shiftin gon [1,7]  JZ 5/24/2012
    ...
    methods % Initialization
        function obj = BehaviorBox14(varargin)
            ...
            obj.default = struct(...
                ...
                'punishTime',10,...%
                'laserTime',5,...% add laserTime so long that it will stop only after the touch 12/17/11
                ...
                'toneDuration',.5,...
                'toneVolume',.5,...
                'LaserDelayTime',4);%delay the laser JZ 5/20/12
            obj.isready = false;
    ...
    methods % User Control Functions
        function start(obj)    
            ...
            obj.pauseTimer = timer(...
             ...
             obj.laserTimer = timer(...% laser timer setting. either end when time is up, or stop when call the stop function 11/20/11
                ...
                'TasksToExecute',2);
             obj.LaserDelayTimer = timer(...% delay laser JZ 5/20/12
                'StartFcn',@(src,evnt)startLaserDelayFcn(obj,src,evnt),...
                'StartDelay',obj.LaserDelayTime,...
                'TimerFcn',@(src,evnt)endLaserDelayFcn(obj,src,evnt),...
                'TasksToExecute',2);
            fprintf('\n\n')
        ...
        function stop(obj)            
            ...
            delete(obj.LaserDelayTimer);%5/20/12
...
%                            if mod(obj.ReversalN,2)==1 &&  obj.counterCurrent + obj.counterP >= 2 %control the laser burst only at the time of 2 more correct and in even reversal JZ 12/20 
%                             
...
%                  if mod(obj.ReversalN,2)==1 &&  obj.counterCurrent + obj.counterP >= 2 %control the laser burst only at the time of 2 more correct and in even reversal JZ 2/20 
...
%             is reached without a response from the mouse
        ...
        function startLaserDelayFcn(obj,src,evnt)% JZ 5/20/12
          
        end
        function endLaserDelayFcn(obj,src,evnt)% JZ 5/20/12
                start(obj.laserTimer);% 11/20/11
...
%                 end;
               if mod(obj.ReversalN,2)==1 &&  obj.counterCurrent + obj.counterP >= 2 %control the laser burst only at the time of 2 more correct and in even reversal JZ 12/20 
                            ...
                            start(obj.LaserDelayTimer);% 11/20/11
               end
        ...
        function startPunishFcn(obj,src,evnt)
            ...
            if mod(obj.ReversalN,2)==1 &&  obj.counterCurrent + obj.counterP >= 2 %control the laser burst only at the time of 2 more correct and in even reversal then start laser delay JZ 5/20/12 
