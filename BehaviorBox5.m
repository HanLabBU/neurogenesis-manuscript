classdef BehaviorBox5 < hgsetget
    % ---------------------------------------------------------------------
    % BehaviorBox
    % Han Lab
    % 7/11/2011
    ...
    %
    %
     %JZ changed to add touchtone, punishlight and
    %initialization sound. Currently, punishment time matches with ITI,so new trial starts after
    % punishment. If we want, we can set different paremeter, so ITI doesn't have to match punishment 
    % ITI is the time between reward giving and the initializaion light turn on for initialze. 10/03/2011
    ...
    methods % Initialization
        function obj = BehaviorBox5(varargin)
            ...
            obj.default = struct(...
                'stimOnTime',2000,...
                'interTrialInterval',5,...%% ITI had better to match the punishtime and determine how long to start the WaitForInitialzation stage,  JZ 9/20/11
                'pauseTime',0.5,...
                'rewardTime',.2,...
                'punishTime',5,...
    ...
    methods % User Control Functions
        function start(obj)
            ...
            obj.interTrialTimer = timer(...
                ...
                'TasksToExecute',1);
            obj.punishmentTimer = timer(...%  JZ 9/20/11
            ...
            obj.rewardPump('on') % works in reverse. so ON == OFF
            nstim = size(obj.stimSet,1);% JZ 10/3/11{
            switch obj.stimOrder
                ...
                case 'random'
                   obj.currentStimNumber = randsample(7,2)';%%% to get 2 random numbers out of 7 & put into 1 by k vectorJZ 9-14-11 %obj.currentStimNumber = ceil(nstim*rand);
            end% JZ 10/3/11}
            obj.currentStim = obj.currentStimNumber; %% to get 2 random numbers JZ9/21/11 %obj.currentStim = obj.stimSet{obj.currentStimNumber};
        ...
        function stop(obj)            
            ...
            delete(obj.punishmentTimer);%% JZ 9/20/11
    ...
    methods % Hardware Control Functions
        ...
        function givePunishment(obj,varargin)
            if obj.isready
                ...
                obj.playSound(500,0.25);% JZ 9/20/11 indicating the success poke
            end
            obj.houseLight('off') % works in reverse. so ON == OFF JZ 9/20/11
            start(obj.punishmentTimer);%%JZ 9/20/11
        end
        ...
        function falsePokeFcn(obj,src,evnt)
            if obj.isready
                if strcmp('stimulus',obj.trialPhase)
                    obj.mouseResponse = eval(src.name(end)); % e.g. name = 'rectangle5' -> mouseResponse = 5
                    fprintf('\b %d \n',obj.mouseResponse)
                    stop(obj.stimulusTimer);%% JZ 9/20/11
                    obj.givePunishment;%% JZ 9/20/11
                    %stop(obj.stimulusTimer);
                end
            end
        end
    ...
    methods % Time-Point Functions
        ...
        function stopStimFcn(obj,src,evnt)
            ...
            if ~isempty(obj.mouseResponse)
                % Mouse Responded
                 switch obj.mouseResponse  %% match with one of 2 stimuli JZ 9/21/11% obj.mouseResponse == obj.correctResponse{obj.currentStimNumber}
...
%                          start(obj.pauseTimer);
                    ...
                    obj.shouldContinue = true;
                    % To count the thinking time in each trial, we need to start each trial with nosepoke initialization, even after punishment 
                    start(obj.interTrialTimer);%%% start the next trial after punishment!! not find in the Box 4 JZ 9/20/11
                end
            else
                % Mouse didn't respond
                obj.shouldContinue = true;
                stop(obj.stimulusTimer);% found in Box4 but not  originally JZ 10/3/2011      
        ...
        function endInterTrialFcn(obj,src,evnt)
            ...
            switch obj.stimOrder
                ...
                case 'random'
                      obj.currentStimNumber = randsample(7,2)';%%% to get 2 random numbers out of 7 & put into 1 by k vectorJZ 9-21-11 %obj.currentStimNumber = ceil(nstim*rand);
            end
             obj.currentStim = obj.currentStimNumber; %% to get 2 random numbers JZ9/21/11 % obj.stimSet{obj.currentStimNumber};
            obj.touchDisplayObj.prepareNextStimulus(obj.currentStim); % e.g. currentStim = [3 5]
            % Transition to Wait4Poke Phase
            if ~obj.shouldContinue
                obj.trialPhase = 'wait4poke';
                obj.rewardLight('on')% different from BOX4 JZ 10/3/11
        ...
        function startPunishFcn(obj,src,evnt)%renamed to match the Timer JZ 10/3/2012
            obj.houseLight('off') % works in reverse. so ON == OFF  %% JZ 9/20/11
        end
        function endPunishFcn(obj,src,evnt)%renamed to match the Timer JZ 10/3/2012
            % Prepare the Next Stimulus
%             nstim = size(obj.stimSet,1);%JZ 10/3/11{
%             switch obj.stimOrder
%                 case 'sequential'
...
%             obj.currentStim = obj.stimSet{obj.currentStimNumber};
            %obj.touchDisplayObj.prepareNextStimulus(obj.currentStim); %
            %e.g. currentStim = [3 5]% Punishment should not trigger the
            %next trial in BOX5 JZ 10/3/11

            obj.houseLight('on') % works in reverse. so ON == OFF %JZ 10/3/2011
            % Transition to Wait4Poke Phase %JZ 10/3/11}
            if ~obj.shouldContinue
                obj.trialPhase = 'wait4poke';
                obj.rewardLight('on')
            else % the nose-poke was poked during the ITI
            ...
            fprintf('%s',datestr(now,'HH:MM:SS'))
            % notify(obj,'TrialStart')JZ 10/3/2011
        end
        
        function startPauseFcn(obj,src,evnt)
             obj.playSound(500,0.25);%% JZ 9/20/11
    ...
    methods % Cleanup
        function delete(obj)
            ...
            delete(obj.stimulusTimer)
            delete(obj.interTrialTimer)
            delete(obj.punishmentTimer)% JZ 10/3/2011
